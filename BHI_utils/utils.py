...
def adapt_to_target(hyper_model, x_trn, y_trn, target_study, model_path, kseed=None, suffix='',
    ...
    target_ids = kwargs['studies'] == target_study
    x_target, y_target = x_trn[target_ids], y_trn[target_ids]
    return fine_tune(x_target, y_target, hyper_model=hyper_model, project_dir=model_path, kseed=kseed,
...
def fine_tune_mc_dropout(hyper_model, x_source, y_source, model_path, next_hyper_model, kseed, **kwargs):
    ...
    def select_fn(model, x, y, hp, **kwargs):
        return MonteCarloSelection(model, x, y, hp, **get_best_trial(tuner).hyperparameters.values)
